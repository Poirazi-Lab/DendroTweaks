...
class Section(Node):
    ...
    def _plot_radii_distribution(self, ax, plot_parent, section_color, parent_color):
        ...
        if hasattr(self, '_ref') and self._ref:
            ...
            bar_width = [self.L / self.nseg] * self.nseg
        ...
        if plot_parent and self.parent:
            ...
            if hasattr(self.parent, '_ref') and self.parent._ref:
                ...
                parent_bar_width = [self.parent.L / self.parent.nseg] * self.parent.nseg
...
class JaxleySection(Section):
    ...
    def create_and_reference(self):
        ...
        """
        self._ref = jx.modules.branch.Branch()
        self._nseg = self._ref.ncomp
        if self.parent is not None:
            self._ref.branch_edges = pd.DataFrame(
                dict(
                    parent_branch_index=[self.parent.idx],
                    child_branch_index=[child.idx for child in self.children]
                )
            )
